 ...
 * 1.1			01.21.2019	Modified to work with HiveControl
 * 
 ******************************/
...
void write2file(char *outputFile, struct tm tm, struct bme680_field_data data)
{
	...
	if(outputFile != NULL)
	{
		...
		else
		{
			...
			fprintf(f,"%.2f %.2f %.2f", data.temperature / 100.0f, data.pressure / 100.0f, data.humidity / 1000.0f );
...
int main(int argc, char *argv[] )
{
	...
	fprintf(f,"Error I2C locked by BME680 readout. \r\n");
	fclose(f);
	...
	while(i<nMeas && backupCounter < nMeas+5) {
		...
		if(data.status & BME680_HEAT_STAB_MSK)
		{
			...
			tempC = data.temperature / 100.0f; 
			tempF =  tempC * 9 / 5 + 32;

			printf("%.2f %.2f %.2f %.2f %d\n", tempF, tempC, data.humidity / 1000.0f, data.pressure / 100.0f, data.gas_resistance );
